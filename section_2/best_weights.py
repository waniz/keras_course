...
# determinate the seed

model = Sequential()
model.add(Dense(input_dim=train_dataset.shape[1], output_dim=train_dataset.shape[1], activation='relu'))
model.add(Dense(4, activation='relu'))  # 67 % - 78%
# model.add(Dense(4, activation='relu'))  # 67.53 %
model.add(Dense(output_dim=1, activation='sigmoid'))
best_loss = ModelCheckpoint('saves/the_best_model.h5', monitor='val_loss', verbose=1, save_best_only=True)
model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
model.fit(x_train, y_train, nb_epoch=300, batch_size=10, validation_data=(x_test, y_test), callbacks=[best_loss])
...
trained_model.load_weights('saves/the_best_model.h5')
trained_model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
