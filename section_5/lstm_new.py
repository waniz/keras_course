...
def get_data():
    ...
    sentences = []
    next_chars = []
    for i in range(0, len(raw_text_ru) - max_len, step):
        sentences.append(raw_text_ru[i: i + max_len])
        next_chars.append(raw_text_ru[i + max_len])
    ...
    return raw_text_ru, max_len, len(chars), char_to_indices, indices_to_chars, x, y
